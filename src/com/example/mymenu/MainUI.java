	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		if (isleftrightEvent) {
			...
			case MotionEvent.ACTION_MOVE:
				int curScrollX = getScrollX();// 定义curScrollX为X方向滚动距离
				int dis_x = (int) (ev.getX() - point.x);// dis_x滑动距离
				int expectX = -dis_x + curScrollX;// expectX差值
				int finalX = 0;// 最终X值
				if (expectX < 0) {
					finalX = Math.max(expectX, -leftMenu.getMeasuredWidth());// 向左滚动
				} else {
					finalX = Math.min(expectX, rightMenu.getMeasuredWidth());// 向右滚动
				}
				scrollTo(finalX, 0);// 移动到这个位置
				point.x = (int) ev.getX();
				break;

			case MotionEvent.ACTION_UP:
			case MotionEvent.ACTION_CANCEL:// 滑出边界时的事件
